...
public class RNSoundModule extends ReactContextBaseJavaModule implements AudioManager.OnAudioFocusChangeListener {
  ...
  Boolean wasPlayingBeforeFocusChange = false;
	String playerFocusType=""; //Added by chandrajyoti
  ...
  @ReactMethod
  public void play(final Double key, String focusType, final Callback callback) {
		this.playerFocusType = focusType;
    ...
    if (!this.mixWithOthers) {
      ...
      switch (this.playerFocusType) { // Extra parameter "focusType" to this method & this switch condition added by Chandrajyoti
        case "GAIN":
          audioManager.requestAudioFocus(mAudioFocusListener, AudioManager.STREAM_MUSIC, AudioManager.AUDIOFOCUS_GAIN);    //mAudioFocusListener is added by chandrajyoti so that after palying sound give back the audio focus to the other app
          break;
        case "TRANSIENT":
          audioManager.requestAudioFocus(mAudioFocusListener, AudioManager.STREAM_MUSIC, AudioManager.AUDIOFOCUS_GAIN_TRANSIENT);
          break;
        case "EXCLUSIVE":
          audioManager.requestAudioFocus(mAudioFocusListener, AudioManager.STREAM_MUSIC, AudioManager.AUDIOFOCUS_GAIN_TRANSIENT_EXCLUSIVE);
          break;
        case "MAY_DUCK":
          audioManager.requestAudioFocus(mAudioFocusListener, AudioManager.STREAM_MUSIC, AudioManager.AUDIOFOCUS_GAIN_TRANSIENT_MAY_DUCK);
          break;
        default:
    ...
    player.setOnCompletionListener(new OnCompletionListener() {
      ...
      public synchronized void onCompletion(MediaPlayer mp) {
        ...
        if (!mp.isLooping()) {
					audioManager.abandonAudioFocus(mAudioFocusListener); //Added by chandrajyoti to give back the audio focus to the other app
          setOnPlay(false, key);
          if (callbackWasCalled) return;
          callbackWasCalled = true;
          try {
	...
	private AudioManager.OnAudioFocusChangeListener mAudioFocusListener = new AudioManager.OnAudioFocusChangeListener() {  //Added by chandrajyoti to give back the audio focus to the other app
  ...
  public void release(final Double key) {
    MediaPlayer player = this.playerPool.get(key);
    if (player != null) {
			player.reset(); //Added by chandrajyoti
      player.release();
    ...
    if (!this.mixWithOthers) {
      ...
      if (player != null) {
        if (focusChange <= 0) {
            ...
            if (this.wasPlayingBeforeFocusChange) {
              this.play(this.focusedPlayerKey, this.playerFocusType, null); //this.playerFocusType added by chandrajyoti
